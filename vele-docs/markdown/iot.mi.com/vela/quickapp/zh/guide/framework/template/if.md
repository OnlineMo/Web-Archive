...
-   [简体中文](if.html){.nav-link .router-link-exact-active
    .router-link-active aria-current="page"}
-   [English](https://iot.mi.com/vela/quickapp/en/guide/framework/template/if.html){.nav-link}
:::
:::
:::
:::
...
-   ::: {.section .sidebar-group .depth-0}
    ...
    -   ::: {.section .sidebar-group .collapsable .is-sub-group .depth-1}
        ...
        -   [条件指令](if.html){.active .sidebar-link
            aria-current="page"}
...
# [\#](if.html#条件指令){.header-anchor} 条件指令

## [\#](if.html#if指令){.header-anchor} if指令

if 条件指令，是指 if/elif/else 这 3
个相关指令，用于控制是否增加或者删除组件；
...
## [\#](if.html#show指令){.header-anchor} show指令

show 指令，是指是否显示组件，用于控制组件的显示状态，并不会从 DOM
结构中删除；
...
## [\#](if.html#if与show区别){.header-anchor} if与show区别

-   当 if/elif 指令的值为 false 时，节点会从页面中移除，当 if/elif
    指令值为 true，组件动态插入 DOM 结构中；

-   当 show 指令的值为 true 时，节点可见，当其值为 false
    时，组件不可见，但节点仍会保留在页面 DOM 结构中。
:::

::: page-nav
[ ← [循环指令](for.html){.prev} ]{.prev} [ [计算属性](computed.html) →
]{.next}
:::
:::
...
[if指令](if.html#if指令 "if指令")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[show指令](if.html#show指令 "show指令")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[if与show区别](if.html#if与show区别 "if与show区别")
:::
:::
:::
