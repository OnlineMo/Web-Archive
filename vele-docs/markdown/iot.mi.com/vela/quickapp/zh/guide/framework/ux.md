...
-   [简体中文](ux.html){.nav-link .router-link-exact-active
    .router-link-active aria-current="page"}
-   [English](https://iot.mi.com/vela/quickapp/en/guide/framework/ux.html){.nav-link}
:::
:::
:::
:::
...
-   ::: {.section .sidebar-group .depth-0}
    ...
    -   [UX 文件](ux.html){.active .sidebar-link aria-current="page"}
...
# [\#](ux.html#ux-文件){.header-anchor} UX 文件

APP，页面均通过 ux 后缀文件编写，ux 后缀文件由[template
模板](template/index.html)、[style 样式](style/index.html)和[script
脚本](script/index.html)3 个部分组成，一个典型的页面 ux
后缀文件示例如下：
...
## [\#](ux.html#app-ux){.header-anchor} app.ux {#app-ux}

当前`app.ux`编译后会包含`manifest配置信息`（可以在`npm run build`之后查看文件内容），所以请不要删除`/**manifest**/`的注释内容标识。

您可以在`<script>`中引入一些公共的脚本，并暴露在当前 app
的对象上，如下所示，然后就可以在页面 ux 文件的 ViewModel
中，通过`this.$app.$def.util`访问。
...
::: page-nav
[ ← [项目配置](manifest.html){.prev} ]{.prev} [
[Props](template/Props.html) → ]{.next}
:::
:::
