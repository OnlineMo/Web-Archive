...
-   [简体中文](data-fetch.html){.nav-link .router-link-exact-active
    .router-link-active aria-current="page"}
-   [English](https://iot.mi.com/vela/quickapp/en/guide/start/data-fetch.html){.nav-link}
:::
:::
:::
:::
...
-   ::: {.section .sidebar-group .depth-0}
    ...
    -   [数据获取](data-fetch.html){.active .sidebar-link
        aria-current="page"}
...
# [\#](data-fetch.html#数据获取){.header-anchor} 数据获取

在前面几个章节中，已经实现了天气预报App的UI，也添加了基本的交互。但页面中的数据
并非真实数据，接下来我们将从和风天气预报接口中请求真实数据并渲染。

## [\#](data-fetch.html#请求接口数据){.header-anchor} 请求接口数据
...
## [\#](data-fetch.html#数据渲染){.header-anchor} 数据渲染
...
::: page-nav
[ ← [添加交互](add-interactivity.html){.prev} ]{.prev} [
[编译参数](toolkit-params.html) → ]{.next}
:::
:::
...
[请求接口数据](data-fetch.html#请求接口数据 "请求接口数据")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[数据渲染](data-fetch.html#数据渲染 "数据渲染")
:::
:::
:::
