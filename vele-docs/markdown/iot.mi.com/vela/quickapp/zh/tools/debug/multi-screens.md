...
-   [简体中文](multi-screens.html){.nav-link .router-link-exact-active
    .router-link-active aria-current="page"}
-   [English](https://iot.mi.com/vela/quickapp/en/tools/debug/multi-screens.html){.nav-link}
:::
:::
:::
:::
...
-   ::: {.section .sidebar-group .depth-0}
    ...
    .clickable .open}
    -   [编译预览](start.html){.sidebar-link}
    -   [日志查看](watch-log.html){.sidebar-link}
    -   [调试运行](debug.html){.sidebar-link}
    -   [功能按钮](toolbar.html){.sidebar-link}
    -   [编译设置](build-setting.html){.sidebar-link}
    -   [内存分析](memory.html){.sidebar-link}
    -   [多屏适配](multi-screens.html){.active .sidebar-link
        aria-current="page"}
    -   [优化评分](audit.html){.sidebar-link}
    :::
...
-   ::: {.section .sidebar-group .depth-0}
    ...
    .clickable}
    -   [AIoT-toolkit](../toolkit/start.html){.sidebar-link}
    -   [升级迁移](../toolkit/update.html){.sidebar-link}
    :::
...
# [\#](multi-screens.html#多屏适配){.header-anchor} 多屏适配

为了提升开发者开发体验，`AIoT-IDE`内置了\*\*\*\*功能，可将一套代码跑在多个不同的设备屏幕上，以测试在不同屏幕上的适配效果。

## [\#](multi-screens.html#多设备模拟器创建){.header-anchor} 多设备模拟器创建
...
![alt
text](../../../images/tools/s3-watch.png){style="max-width:100%;height:auto;"}
...
::: {style="width:20%;text-align:center;"}
![alt
text](../../../images/tools/s4-watch.png){style="max-width:100%;height:auto;"}
...
::: {style="width:20%;text-align:center;"}
![alt
text](../../../images/tools/8-band.png){style="max-width:100%;height:auto;"}
...
::: {style="width:20%;text-align:center;"}
![alt
text](../../../images/tools/8pro-band.png){style="max-width:100%;height:auto;"}
...
::: {style="width:20%;text-align:center;"}
![alt
text](../../../images/tools/10-band.png){style="max-width:100%;height:auto;"}
...
::: {style="width:20%;text-align:center;"}
![alt
text](../../../images/tools/sound-mini.png){style="max-width:100%;height:auto;"}
...
## [\#](multi-screens.html#自定义模拟器){.header-anchor} 自定义模拟器
...
## [\#](multi-screens.html#多屏预览){.header-anchor} 多屏预览
...
## [\#](multi-screens.html#自定义模拟器皮肤){.header-anchor} 自定义模拟器皮肤

此外，如果用户不仅仅满足于自定义模拟器的显示效果，希望能更逼真的预览模拟器设备，我们还提供了自定义皮肤的功能。

## [\#](multi-screens.html#vela-模拟器皮肤组){.header-anchor} Vela 模拟器皮肤组
...
其中`foreground.png`
一般由背景图片中扣出模拟器画面部分并将屏幕部分修改为透明色。没有
`foreground.png` 与有 `foreground.png` 的对比如下:

::: {style="margin-top:20px;display:flex;justify-content:center;align-items:center;width:100%;"}
::: {style="width:50%;text-align:center;"}
![alt
text](../../../images/tools/ide-skin-2.png){style="max-width:100%;height:auto;"}
...
::: {style="width:50%;text-align:center;"}
![alt
text](../../../images/tools/ide-skin-3.png){style="max-width:100%;height:auto;"}

配置了foreground.png
:::
:::

## [\#](multi-screens.html#layout-文件的制作){.header-anchor} Layout 文件的制作
...
## [\#](multi-screens.html#应用皮肤文件){.header-anchor} 应用皮肤文件

在通过通过 `AIoT-IDE` 打开 `SKD` 目录，该目录下有一个 **skins** 目录，在
**skins/user**
目录下新建一个文件夹名称为你的皮肤名称，将制作好的文件放入其中，如果
skins下没有user，则先创建 user 目录 ，再次使用 IDE
创建模拟器时即可选择应用该皮肤。
...
## [\#](multi-screens.html#layout详解){.header-anchor} layout详解
...
-   **layout** 中可定义 **portrait** 和 **landscape**
    两种布局方式，即竖屏和横屏其中的**width** 和 **height**
    为你的**background.png** 的像素大小

-   **event** 固定值填 **EV_SW:0:0**

-   **part1** 引用上方 parts 中定义背景图片和前景图的那个，name 表示
    引用 part 的名称，x，y 表示布局开始的左边，以左上角开始为0，0

-   **part2** 引用上方 parts 中定义 **dispaly** 的那个，name 表示 引用
    part 的名称，x，y 表示布局开始的坐标，以左上角开始为0，0

-   **parts**
    定义皮肤的组成部分，一般又两部分组成，即皮肤和模拟器画面，带
    **display** 的表示模拟器画面，带 **background** 和 **foreground**
    的表示皮肤。
:::

::: page-nav
[ ← [内存分析](memory.html){.prev} ]{.prev} [ [优化评分](audit.html) →
]{.next}
:::
:::
...
[多设备模拟器创建](multi-screens.html#多设备模拟器创建 "多设备模拟器创建")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[自定义模拟器](multi-screens.html#自定义模拟器 "自定义模拟器")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[多屏预览](multi-screens.html#多屏预览 "多屏预览")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[自定义模拟器皮肤](multi-screens.html#自定义模拟器皮肤 "自定义模拟器皮肤")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[Vela
模拟器皮肤组](multi-screens.html#vela-模拟器皮肤组 "Vela 模拟器皮肤组")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[Layout
文件的制作](multi-screens.html#layout-文件的制作 "Layout 文件的制作")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[应用皮肤文件](multi-screens.html#应用皮肤文件 "应用皮肤文件")
:::

::: {.vuepress-toc-item .vuepress-toc-h2}
[layout详解](multi-screens.html#layout详解 "layout详解")
:::
:::
:::
