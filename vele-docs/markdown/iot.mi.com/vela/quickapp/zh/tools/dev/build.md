...
-   [简体中文](build.html){.nav-link .router-link-exact-active
    .router-link-active aria-current="page"}
-   [English](https://iot.mi.com/vela/quickapp/en/tools/dev/build.html){.nav-link}
:::
:::
:::
:::
...
-   ::: {.section .sidebar-group .depth-0}
    ...
    -   [应用热更新](build.html){.active .sidebar-link
        aria-current="page"}
...
# [\#](build.html#应用热更新){.header-anchor} 应用热更新

在使用`AIoT-IDE`开发**Xiaomi Vela JS
应用**时，还内置了热更新功能。开发者在通过点击**顶部操作栏**`运行`或`调试`按钮，运行模拟器时会时刻监听文件变化。
...
::: page-nav
[ ← [代码美化](format.html){.prev} ]{.prev} [
[可视化编辑](manifest.html) → ]{.next}
:::
:::
:::
